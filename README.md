...
### Введение в контекст решаемых библиотекой задач:
...
Таким образом, каждому слову в соответствие можно ппоставить лемму + набор всех изменений этой леммы, которые возникают когда мы меняем морфологические характеристики (род, падеж, число). 
Эти изменения называются *флексиями*.
Если у нас есть строка некоего слова, то ему могут соответствовать несколько возможных слов (пар: лемма + флексии леммы).
...
```  
Каждое из трех выше перечисленных слов являет
